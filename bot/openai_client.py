...
FINAL_MESSAGE_PROMPT = """Сформируй финальное подтверждение брони для гостя.

Данные брони (JSON):
{structured_json}

Напиши одно короткое дружелюбное сообщение в таком стиле:
«Бронирование оформлено: [дата и время по-русски], [N] гостей, [этаж], [с/без сертификата]. Если есть пожелания, напишите их в комментарии к брони. Ждём вас!»
...
def generate_booking_confirmation(
    ...
    """Один вызов LLM: генерирует финальное подтверждение брони по данным."""
    client_kwargs: Dict[str, Any] = {"api_key": api_key, "timeout": 30}
    if base_url:
        client_kwargs["base_url"] = base_url
    client = OpenAI(**client_kwargs)
    prompt = FINAL_MESSAGE_PROMPT.format(structured_json=json.dumps(structured, ensure_ascii=False, indent=2))
    try:
        ...
        return content if content else _fallback_reply(structured)
    except Exception:
        return _fallback_reply(structured)


def _fallback_reply(structured: Dict[str, Any]) -> str:
    date_s = structured.get("date", "—")
    time_s = structured.get("time", "—")
    guests = structured.get("guests_count", "—")
    floor = structured.get("floor", "—")
    cert = "с сертификатом" if structured.get("certificate_needed") else "без сертификата"
    return (
        f"Бронирование оформлено: {date_s} в {time_s}, {guests} гостей, этаж {floor}, {cert}. "
        "Если есть пожелания, напишите их в комментарии к брони. Ждём вас!"
    )
