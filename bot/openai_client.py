...
Цель: помочь забронировать. Для брони нужны: дата, время, гости.
Спрашивай параметры по одному: сначала дату, потом время, потом число гостей, желательный этаж и нужен ли подарочный сертификат. Никогда не проси несколько полей сразу (не «дата и время», а только «дата», затем отдельно «время»).
...
def _parse_time_fallback(text: str) -> Optional[str]:
    """Извлекает время из текста (15:30, 4 часа, 5 вечера, 17)."""
...
Правила:
- intent "booking_param": гость указывает параметр брони. ИЗВЛЕКИ ВСЕ УПОМЯНУТЫЕ параметры. Если гость написал «завтра в 15:30» — верни и date_text, и time_text. Поля: date_text (YYYY-MM-DD), time_text (HH:MM), guests_count_text (число), name_text, phone_text, floor_text (1/2), certificate_needed_text (true/false). booking_update может быть объектом или массивом объектов [{"field":"date_text","value":"..."},{"field":"time_text","value":"15:30"}].
